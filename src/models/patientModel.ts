...
class PatientModel {
  ...
  public static async getAllPatientsByEmail(
    email: string
  ): Promise<IPatient[]> {
    const patients = await db(this.table)
      .where("email", email)
      .returning("*");
    return patients;
  }
  public static async createPatient(
    patient: IPatientToInsert
  ): Promise<IPatient> {
  ...
  public static async updatePatientByTokenEmail(
    patient: IPatient
  ): Promise<IPatient> {
    const updatedPatient = await db(this.table)
  ...
  public static async deletePatient(id: number): Promise<IPatient> {
    const deletedpatient = await db(this.table)
      .where("id", id)
      .del()
      .returning("*");
    return deletedpatient[0];
  }
  public static async deletePatientByTokenEmail(
    id: number,
    email: string
  ): Promise<IPatient> {
    const deletedPatient = await db(this.table)
